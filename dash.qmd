...
df = pl.DataFrame({
  'Precinct': i,
  'Target': [100*i for i in i],
  'Voted': [10*i for i in i],
  'Reach Commit': [80,4,3,200,1],
  'VAN Commit': [3,9,8,4,2]
})
...
df1_agg = (
  df
  .with_columns( Precinct = pl.lit('Total') )
  .group_by(['Precinct'])
  .agg( cs.all().sum() )
  .with_columns( 
    pl.col('Voted').alias('Voted Percent of Goal')/
